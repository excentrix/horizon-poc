...
from sqlmodel import SQLModel, Field, Relationship, create_engine, Session, select, Column, String, Integer, DateTime, Boolean, Text
...
def create_db_and_tables():
    ...
    try:
        ...
        print("📋 Horizon tables: chat_sessions, chat_messages, tasks, fact_extractions")
...
class DatabaseService:
    ...
    @staticmethod
    def create_chat_session(user_id: str, title: str = "New Conversation") -> ChatSession:
        with Session(engine) as session:
            try:
    ...
    @staticmethod
    def add_message(session_id: str, content: str, is_user: bool, metadata: dict = None) -> ChatMessage:
        with Session(engine) as session:
            try:
                message = ChatMessage(
    ...
    @staticmethod
    def create_task(user_id: str, title: str, description: str = None, due_date: datetime = None, created_by_ai: bool = False) -> Task:
        with Session(engine) as session:
            try:
                task = Task(
                    user_id=user_id,
